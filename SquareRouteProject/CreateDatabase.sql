...
IF EXISTS (SELECT 1 FROM sys.databases WHERE [Name] = 'SquareRouteProject')
BEGIN
    ALTER DATABASE SquareRouteProject SET SINGLE_USER
    DROP DATABASE SquareRouteProject
END

CREATE DATABASE SquareRouteProject
GO

IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE [name] = 'SquareRouteProjectUser')
BEGIN
    CREATE LOGIN [SquareRouteProjectUser] WITH PASSWORD = N'Password123', DEFAULT_DATABASE = [SquareRouteProject],
        DEFAULT_LANGUAGE=[us_english], CHECK_EXPIRATION = OFF, CHECK_POLICY = OFF
    
    ALTER LOGIN [SquareRouteProjectUser] ENABLE
END
GO

USE [SquareRouteProject]
GO

CREATE USER [SquareRouteProjectUser] FOR LOGIN [SquareRouteProjectUser]
GO

EXEC sp_addrolemember N'db_datareader', N'SquareRouteProjectUser'
EXEC sp_addrolemember N'db_datawriter', N'SquareRouteProjectUser'
GO
